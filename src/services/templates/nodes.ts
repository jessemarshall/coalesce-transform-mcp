  ...
  configGroups.forEach((group, groupIndex) => {
    ...
    items.forEach((item, itemIndex) => {
      ...
      targetMappings.forEach(({ targetPath, note }) => {
        ...
        if (!targetPath) {
          warnings.push(
            `Config item ${groupIndex}.${itemIndex} (${getString(item.type) ?? "unknown"}) does not map cleanly to set-workspace-node body fields.`
          );
          return;
        }
        ...
        if (defaultValue === undefined) {
          warnings.push(
            `Config item ${targetPath} has no inferred default. Fill it before calling set-workspace-node if the node type requires it.`
          );
          return;
        }
