  ...
  try {
    ...
    if (!isTopRanked && topRanked) {
      validation.warning =
        ...
        `Call coalesce_plan_pipeline first to discover and rank all available node types.`;
    }
    ...
    if (specializedPenalty) {
      ...
      throw new Error(
        ...
        `Call coalesce_plan_pipeline to discover the correct nodeType for your use case.`
      );
    }
...
function assertNotSourceNodeType(nodeType: string): void {
  ...
  if (normalized === "source") {
    throw new Error(
      ...
      `Call coalesce_plan_pipeline to discover the correct nodeType.`
    );
  }
}
...
export async function replaceWorkspaceNodeColumns(
  ...
  if (params.additionalChanges) {
    ...
    if (additionalMeta && ("sourceMapping" in additionalMeta || "customSQL" in additionalMeta)) {
      throw new Error(
        "coalesce_replace_workspace_node_columns additionalChanges cannot set sourceMapping or customSQL. " +
        "Use the joinCondition parameter to set WHERE filters, coalesce_apply_join_condition for join setup, " +
        "or coalesce_convert_join_to_aggregation for GROUP BY patterns."
      );
    }
  }
...
function buildScratchNodeNextSteps(
  ...
  if (columns.length === 0) {
    steps.push(
      "This node has no columns. Add columns using coalesce_replace_workspace_node_columns or coalesce_update_workspace_node."
    );
  }
  ...
  // Verification
  steps.push(
    "Verify the node: call coalesce_get_workspace_node to confirm columns and config are correct before proceeding."
  );
  ...
  if (!validation.completionSatisfied) {
    if (completionLevel === "configured") {
      throw new Error(
        ...
        `To clean up, use coalesce_delete_workspace_node with nodeID "${created.id}".`
      );
    }
    return {
      node: finalNode,
...
function buildPostCreationNextSteps(
  ...
  if (predecessorCount > 1) {
    ...
    steps.push(
      ...
      "\n  - Call coalesce_convert_join_to_aggregation (for GROUP BY / aggregation use cases)" +
      "\n  - Call coalesce_apply_join_condition (for row-level joins — auto-generates FROM/JOIN/ON with {{ ref() }} syntax)"
    );
  ...
  if (family === "fact" || family === "dimension") {
    ...
    if (family === "fact" && predecessorCount > 1) {
      steps.push(
        ...
        "These grain columns become your GROUP BY columns in coalesce_convert_join_to_aggregation. " +
  ...
  // Verification
  steps.push(
    "Verify the node: call coalesce_get_workspace_node to confirm columns, config, and join condition are correct before proceeding to downstream nodes."
  );
...
export async function createWorkspaceNodeFromPredecessor(
  ...
  if (params.whereCondition && params.groupByColumns) {
    throw new Error(
      ...
      "For aggregation nodes, WHERE/HAVING filters should be applied via a separate coalesce_update_workspace_node call."
    );
  }
...
export async function applyJoinCondition(
  ...
  if (predecessorNodeIDs.length < 2) {
    throw new Error(
      "coalesce_apply_join_condition requires a node with 2+ predecessors. " +
      "This node has " + predecessorNodeIDs.length + " predecessor(s). " +
      "For single-predecessor nodes, set the joinCondition directly via coalesce_update_workspace_node."
    );
  }
  ...
  for (const { nodeID, node: predecessor } of fetchedPredecessors) {
    ...
    if (!refInfo) {
      ...
      warnings.push(
        ...
        `Set it in the Coalesce UI or via coalesce_update_workspace_node before applying joins.`
      );
      continue;
    }
    predecessorRefInfos.push(refInfo);
