...
export function walkUpstream(cache: LineageCacheEntry, startNodeID: string): LineageGraphNode[] {
...
export function walkDownstream(cache: LineageCacheEntry, startNodeID: string): LineageGraphNode[] {
  const result: LineageGraphNode[] = [];
  const visited = new Set<string>();
  const queue: Array<{ nodeID: string; depth: number }> = [{ nodeID: startNodeID, depth: 0 }];
