...
function buildRepoResolutionError(
  ...
  return new Error(
    `Repo-backed definition could not be resolved for ${requestedNodeType} in ${parsedRepo.summary.resolvedRepoPath}. ${detail} Use the corpus tools (coalesce_search_node_type_variants, coalesce_get_node_type_variant, or coalesce_generate_set_workspace_node_template_from_variant) as the next step.`
  );
}
