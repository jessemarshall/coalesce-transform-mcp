    ...
    return {
      ...
      warnings: [
        `Observed workspace node types could not be fetched for workspace ${workspaceID} (${reason}). ` +
          `Use coalesce_list_workspace_node_types or coalesce_cache_workspace_nodes to inspect current workspace usage and confirm installation before execution.`,
      ],
    };
  }
}
...
function buildPlanFromSql(
  ...
  const plan: PipelinePlan = {
    ...
    assumptions: [
      ...
      "The generated plan uses coalesce_create_workspace_node_from_predecessor followed by coalesce_set_workspace_node when the selected type is projection-capable.",
    ],
...
function buildCteNodeInstruction(cte: ParsedCte, nodeType: string): string {
  ...
  if (cte.hasGroupBy) {
    lines.push(`- AGGREGATION NODE: pass groupByColumns + aggregates directly to coalesce_create_workspace_node_from_predecessor (single call)`);
  } else if (cte.columns.length > 0) {
    lines.push(`- Pass columns array + whereCondition directly to coalesce_create_workspace_node_from_predecessor (single call)`);
  }
  ...
  if (cte.hasJoin) {
    lines.push(`- Has JOIN — use coalesce_apply_join_condition or coalesce_update_workspace_node for join setup`);
  }
...
function buildCtePlan(
  ...
  return {
    ...
    assumptions: [
      ...
      `Staging and aggregation CTEs: 1 call per node. Multi-source JOIN CTEs: 2 calls (create + coalesce_apply_join_condition).`,
    ],
    openQuestions: [
      ...
      `Node type guidance (do NOT use coalesce_list_workspace_node_types):\n` +
      ...
      `Workflow per CTE:\n` +
        `coalesce_create_workspace_node_from_predecessor accepts columns, whereCondition, groupByColumns, and aggregates directly:\n` +
        ...
        `- For multi-source JOIN CTEs: 2 calls — first coalesce_create_workspace_node_from_predecessor with columns + whereCondition, then coalesce_apply_join_condition to set up FROM/JOIN/ON\n` +
