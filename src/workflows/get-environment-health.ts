...
function computeHealthScore(
  ...
  if (failedCount >= 3) {
    reasons.push(`${failedCount} failed runs in the last 24 hours`);
  }
  if (failedNodeCount > totalNodes * 0.5) {
    reasons.push(
  ...
  if (failedCount > 0 && failedCount < 3) {
    reasons.push(`${failedCount} failed run(s) in the last 24 hours`);
  }
  if (staleCount > 0) {
  ...
  if (failedNodeCount > 0 && failedNodeCount <= totalNodes * 0.5) {
    reasons.push(
      `${failedNodeCount}/${totalNodes} nodes have a failed last run`
    );
  }
...
export function registerGetEnvironmentHealth(
  server: McpServer,
  client: CoalesceClient
): void {
  server.registerTool(
    "get_environment_health",
    {
      ...
      outputSchema: getToolOutputSchema("get_environment_health"),
      annotations: READ_ONLY_ANNOTATIONS,
    },
    async (params) => {
      try {
        const result = await getEnvironmentHealth(client, params);
