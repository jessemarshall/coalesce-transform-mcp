...
export async function generateSetWorkspaceNodeTemplate(
  ...
  if (usingDefinition) {
    ...
    if (!isPlainObject(definition.nodeDefinition)) {
      ...
      throw new Error(
        `Repo-backed definition could not be resolved for template generation because metadata.nodeMetadataSpec could not be parsed. Parse error: ${parseError} Use the corpus tools (search-node-type-variants, get-node-type-variant, or generate-set-workspace-node-template-from-variant) as the next step.`
      );
    }
...
export function registerRepoNodeTypeTools(
  server: McpServer,
  client: CoalesceClient
): void {
  server.tool(
