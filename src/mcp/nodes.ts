...
export function defineNodeTools(
  ...
  defineSimpleTool(client, "get_environment_node", {
    ...
    inputSchema: z.object({
      environmentID: z.string().min(1, "environmentID must not be empty").describe("The environment ID"),
      nodeID: z.string().min(1, "nodeID must not be empty").describe("The node ID"),
    }),
    annotations: READ_ONLY_ANNOTATIONS,
  }, getEnvironmentNode),
  ...
  defineSimpleTool(client, "replace_workspace_node_columns", {
    ...
    inputSchema: z.object({
      workspaceID: z.string().min(1, "workspaceID must not be empty").describe("The workspace ID"),
      nodeID: z.string().min(1, "nodeID must not be empty").describe("The node ID"),
      columns: z.array(WorkspaceNodeColumnInputSchema).min(1, "columns must include at least one column").describe(
  ...
  defineSimpleTool(client, "convert_join_to_aggregation", {
    ...
    inputSchema: z.object({
      workspaceID: z.string().min(1, "workspaceID must not be empty").describe("The workspace ID"),
      nodeID: z.string().min(1, "nodeID must not be empty").describe("The node ID of the join to convert"),
      groupByColumns: z
        .array(z.string().min(1, "groupBy column must not be empty"))
        .min(1, "groupByColumns must include at least one column")
        .describe("Columns to group by (dimensions). Use fully-qualified names like '\"TABLE\".\"COLUMN\"'."),
      aggregates: z
        .array(
          z.object({
            name: z.string().min(1, "aggregate name must not be empty").describe("Column name for the aggregate"),
            function: z.string().min(1, "aggregate function must not be empty").describe("Aggregate function: COUNT, SUM, AVG, MIN, MAX, etc."),
            expression: z.string().min(1, "aggregate expression must not be empty").describe("Expression to aggregate (e.g., 'DISTINCT \"TABLE\".\"COLUMN\"')"),
            description: z.string().optional().describe("Optional column description"),
          })
        )
        .min(1, "aggregates must include at least one aggregate")
  ...
  defineSimpleTool(client, "apply_join_condition", {
    ...
    inputSchema: z.object({
      workspaceID: z.string().min(1, "workspaceID must not be empty").describe("The workspace ID"),
      nodeID: z.string().min(1, "nodeID must not be empty").describe("The node ID of the multi-predecessor node"),
      joinType: z
        .enum(["INNER JOIN", "LEFT JOIN", "RIGHT JOIN", "FULL OUTER JOIN"])
        .optional()
      ...
      joinColumnOverrides: z
        .array(
          z.object({
            leftPredecessor: z.string().min(1, "leftPredecessor must not be empty").describe("Name of the left predecessor node"),
            rightPredecessor: z.string().min(1, "rightPredecessor must not be empty").describe("Name of the right predecessor node"),
            leftColumn: z.string().min(1, "leftColumn must not be empty").describe("Column name in the left predecessor"),
            rightColumn: z.string().min(1, "rightColumn must not be empty").describe("Column name in the right predecessor"),
          })
        )
        .optional()
  ...
  defineSimpleTool(client, "create_workspace_node_from_predecessor", {
    ...
    inputSchema: z.object({
      ...
      predecessorNodeIDs: z
        .array(z.string().min(1, "predecessor node ID must not be empty"))
        .min(1)
      ...
      aggregates: z
        .array(
          z.object({
            name: z.string().min(1, "aggregate name must not be empty").describe("Output column name for the aggregate"),
            function: z.string().min(1, "aggregate function must not be empty").describe("Aggregate function: COUNT, SUM, AVG, MIN, MAX, etc."),
            expression: z.string().min(1, "aggregate expression must not be empty").describe("Expression to aggregate"),
  ...
  defineSimpleTool(client, "create_node_from_external_schema", {
    ...
    inputSchema: z.object({
      workspaceID: z.string().min(1, "workspaceID must not be empty").describe("The workspace ID"),
      nodeType: z.string().min(1, "nodeType must not be empty").describe("The type of node to create. Call plan_pipeline first to discover and rank available node types."),
      predecessorNodeIDs: z
        .array(z.string().min(1, "predecessor node ID must not be empty"))
        .min(1)
        .describe("One or more predecessor node IDs to link to the new node"),
  ...
  [
    "analyze_workspace_patterns",
    {
      ...
      inputSchema: z.object({
        workspaceID: z.string().min(1, "workspaceID must not be empty").describe("The workspace ID to analyze"),
      }),
  ...
  defineDestructiveTool(server, client, "delete_workspace_node", {
    ...
    inputSchema: z.object({
      workspaceID: z.string().min(1, "workspaceID must not be empty").describe("The workspace ID"),
      nodeID: z.string().min(1, "nodeID must not be empty").describe("The node ID to delete"),
      confirmed: z
        .boolean()
        .optional()
  ...
  defineSimpleTool(client, "complete_node_configuration", {
    ...
    inputSchema: z.object({
      workspaceID: z.string().min(1, "workspaceID must not be empty").describe("The workspace ID"),
      nodeID: z.string().min(1, "nodeID must not be empty").describe("The node ID to configure"),
