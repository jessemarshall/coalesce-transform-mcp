...
import { defineLocalTool } from "./tool-helpers.js";
...
function personalizeSkills(directory?: string): PersonalizeSkillsResult {
  ...
  // Verify individual files were created (initializeSkillsDir may have partially failed)
  const failedFiles: string[] = [];
  for (const name of created) {
    const skillsFile = join(skillsDir, `coalesce_skills.${name}.md`);
    if (!existsSync(skillsFile)) {
      failedFiles.push(name);
    }
  }
  if (failedFiles.length > 0) {
    const succeeded = created.filter((n) => !failedFiles.includes(n));
    created.length = 0;
    created.push(...succeeded);
    process.stderr.write(
      `[personalizeSkills] Failed to create skill files for: ${failedFiles.join(", ")}. Check disk space and permissions.\n`
    );
  }
...
export function defineSkillTools(_server: McpServer, _client: CoalesceClient): ToolDefinition[] {
  return [
  defineLocalTool("personalize_skills", {
