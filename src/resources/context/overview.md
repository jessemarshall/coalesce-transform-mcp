...
1. **Always call `coalesce_plan_pipeline` first** — it discovers all available node types from the repo and ranks them for your use case. Do NOT guess node types like "Stage" or "View".
2. **Use `coalesce_create_workspace_node_from_predecessor`** (or `coalesce_create_workspace_node_from_scratch` for nodes with no upstream). Pass `repoPath` for automatic config completion.
3. **Use `coalesce_create_workspace_node_from_predecessor`** (or `coalesce_create_workspace_node_from_scratch` for nodes with no upstream) — they handle validation, config completion, and column-level attributes automatically.
...
3. For multi-predecessor nodes: set up the join condition via `coalesce_convert_join_to_aggregation` (aggregation), `coalesce_apply_join_condition` (row-level joins), or `coalesce_update_workspace_node` (manual)
4. Verify the final node with `coalesce_get_workspace_node` — confirm columns, joinCondition, and config are correct
...
Do NOT author SQL yourself to pass to `coalesce_plan_pipeline` or `coalesce_create_pipeline_from_sql`. The `sql` parameter exists solely for converting SQL that the **user** provided. When building a pipeline, use declarative tools:

1. `coalesce_create_workspace_node_from_predecessor` to create nodes
2. `coalesce_update_workspace_node` to set joinCondition
3. `coalesce_replace_workspace_node_columns` or `coalesce_convert_join_to_aggregation` for column transforms

### Writing SQL for Nodes

Determine the platform first, then load exactly one dialect resource:

1. **Detect**: `coalesce_get_project` for warehouse type, or check existing node SQL (see `coalesce://context/sql-platform-selection`)
...
Apply all descriptions in one call using `coalesce_replace_workspace_node_columns`.
