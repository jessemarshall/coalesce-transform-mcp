...
1. Call `list_workspace_node_types` to discover which node types are already observed in current workspace nodes
...
**Before creating any node**, call `plan_pipeline` to discover and rank available node types from the repo:
...
The planner scans the repo for all committed node type definitions, scores them against your use case, and returns `nodeTypeSelection.consideredNodeTypes` with the best match. **Use the `nodeType` from the plan** when calling `create_workspace_node_from_predecessor` — do not guess node types like "Stage" or "View".
...
Use `create_workspace_node_from_predecessor` with the node type from the plan:
...
For joins and aggregations, follow up with `convert_join_to_aggregation`.
...
- prefer `create_pipeline_from_sql` with the user's SQL for one-step conversion
- prefer `create_pipeline_from_plan` when a plan has already been reviewed and approved

Do NOT author SQL yourself to pass to these tools.

### If the node already exists and only part of it should change

Use `update_workspace_node`.
...
2. Use `create_workspace_node_from_predecessor`.
...
6. **Set up the join condition** by calling one of:
   - `convert_join_to_aggregation` — for GROUP BY / fact table / aggregation use cases
   - `apply_join_condition` — for row-level joins (auto-generates FROM/JOIN/ON with `{{ ref() }}` syntax)
   - `update_workspace_node` — to set joinCondition manually when you need full control
7. **Verify the join** — call `get_workspace_node` to confirm the joinCondition is set and columns are correct.
...
When you use `create_workspace_node_from_predecessor` or `create_workspace_node_from_scratch` with `repoPath`, node configuration is completed automatically:
...
If `repoPath` is not provided or config completion fails, the response includes `configCompletionSkipped` — call `complete_node_configuration` with `repoPath` to retry.

**Always use `create_workspace_node_from_predecessor` or `create_workspace_node_from_scratch`** — they handle validation and config completion automatically.
