...
This guide explains how to use the `review_pipeline` tool to analyze existing Coalesce pipelines and identify improvement opportunities.

## When to Use

Use `review_pipeline` when:
...
Do NOT use when:
- You need to create new nodes — use `plan_pipeline` or `build_pipeline_from_intent`
- You need to fix a specific node — use `update_workspace_node` directly
- You need to diagnose a run failure — use `diagnose_run_failure`
...
2. Agent: Call review_pipeline with the workspace ID
3. Agent: Present findings grouped by severity
4. Agent: For critical issues, offer to fix them:
   - Missing join: use apply_join_condition
   - Redundant passthrough: suggest removal
   - Type mismatch: suggest update_workspace_node
...
After reviewing:
- Use `get_workspace_node` to inspect flagged nodes in detail
- Use `apply_join_condition` to fix missing join conditions
- Use `update_workspace_node` to fix node types or configurations
- Use `complete_node_configuration` to fill missing config
- Use `analyze_workspace_patterns` for a higher-level workspace profile
