...
export class LoopbackTransport implements Transport {
  ...
  async send(message: Parameters<NonNullable<Transport["onmessage"]>>[0]): Promise<void> {
    ...
    // Deliver asynchronously via setTimeout(0) instead of queueMicrotask.
    // setTimeout creates an unref-able timer that won't keep the event loop
    // alive, whereas microtasks do.
    const timer = setTimeout(() => {
      if (this._closed || peer._closed) return;
      try {
